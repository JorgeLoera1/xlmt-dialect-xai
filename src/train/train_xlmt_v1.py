...
# Modelo: cardiffnlp/twitter-xlm-roberta-base (XLM-T) + LoRA
# Metodología: EDA → Fine-tuning → Evaluación
# v1: 5 épocas, lr=2e-4, sin WeightedTrainer (referencia para v2)
...
# XLM-T: preentrenado en Twitter multilingüe → capta jerga y lenguaje informal mejor que modelos de texto general
MODEL_NAME      = “cardiffnlp/twitter-xlm-roberta-base”
MAX_LENGTH      = 128
DATA_PATH       = "data/Datos_dialectos.parquet"
SAVE_DIR        = "model_outputs/xlmt_lora_v1"
...
def limpiar_texto_modelo(texto):
    # conserva stopwords y puntuación para mantener el contexto semántico que XLM-T aprovecha
...
# TOKENIZACIÓN
# ============================================================
# XLM-T usa SentencePiece; produce prefijos ▁ en tokens que se filtran en el post-procesamiento XAI
...
class DialectDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # XLM-RoBERTa no usa token_type_ids
        return {
...
# ============================================================
# MODELO XLM-T + LoRA (CONFIGURACIÓN MEJORADA PARA 6GB VRAM)
# ============================================================
base_model = AutoModelForSequenceClassification.from_pretrained(
    ...
    id2label=id2label,
    label2id=label2id,
    ignore_mismatched_sizes=True   # ← la cabeza clasificadora pre-entrenada (distinto num_labels) se reemplaza por una de 21 clases
)

# r=64, alpha=128: mayor rango que la configuración inicial para acomodar 21 clases dialectales
# target_modules incluye dense (FFN) además de atención, captura señales léxicas adicionales
lora_config = LoraConfig(
    task_type=TaskType.SEQ_CLS,
    r=64,                 # subido de 16 → más capacidad para 21 clases
    lora_alpha=128,       # ratio 2:1 con r
    lora_dropout=0.1,
    bias=”none”,
...
model.enable_input_require_grads()   # sin esto, LoRA + gradient_checkpointing produce gradientes None silenciosamente
...
# ============================================================
# ENTRENAMIENTO (OPTIMIZADO PARA RTX 3050 6GB)
# ============================================================
...
training_args = TrainingArguments(
    ...
    num_train_epochs=5,
    # batch pequeño + acumulación para simular batch efectivo = 128 sin exceder 6GB VRAM
    per_device_train_batch_size=8,
    gradient_accumulation_steps=16,       # batch efectivo = 8 × 16 = 128
    per_device_eval_batch_size=32,
    # con LoRA se toleran lr más altas que full FT porque solo se actualizan ~2% de params
    learning_rate=2e-4,
    warmup_ratio=0.1,
    lr_scheduler_type=”cosine”,
    weight_decay=0.01,
    # medidas de ahorro de VRAM para RTX 3050 (6GB)
    fp16=torch.cuda.is_available(),       # reduce uso de memoria ~50%
    gradient_checkpointing=True,          # recalcula activaciones en backward, ahorra VRAM a costa de tiempo
    optim=”paged_adamw_8bit”,             # estados del optimizador en 8-bit, reduce VRAM ~75% vs fp32
    eval_strategy=”epoch”,
    save_strategy="epoch",
...
merged_model = model.merge_and_unload()   # fusiona pesos LoRA en el transformer base para carga estándar HF
...
plt.title("Matriz de Confusión â€” Test Set", fontsize=14, fontweight='bold')
plt.ylabel("Etiqueta Real"); plt.xlabel("Predicción")
plt.tight_layout()
plt.savefig(os.path.join(FIG_TRAIN_DIR, "matriz_confusion_v1.png"), dpi=150, bbox_inches='tight')
plt.close()
...
if eval_data:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_TRAIN_DIR, "curva_aprendizaje_v1.png"), dpi=150, bbox_inches='tight')
    plt.close()
...
print("\n" + "="*60)
print(“ENTRENAMIENTO COMPLETADO -- XLM-T + LoRA v1”)
print(“=”*60)
print(f”””
Modelo:        {MODEL_NAME}
