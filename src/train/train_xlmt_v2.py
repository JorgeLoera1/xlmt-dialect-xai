...
# v2: epochs=10, lr=1e-4, label_smoothing=0.1, fix IG numpy.int64
# ============================================================
...
MAX_LENGTH      = 128
DATA_PATH       = "data/Datos_dialectos.parquet"   # descarga desde data/README.md
SAVE_DIR        = "model_outputs/xlmt_lora_v2"     # ignorado por .gitignore
...
# Dos pipelines distintos: EDA/XAI elimina stopwords para visualización;
# modelo conserva stopwords y puntuación porque el transformer aprovecha ese contexto.
# ============================================================
...
# ============================================================
# CLASS WEIGHTS para clases desbalanceadas (gq tiene menos muestras)
# ============================================================
counts  = df['label'].value_counts().sort_index().values
weights = 1.0 / torch.tensor(counts, dtype=torch.float)
weights = (weights / weights.sum() * NUM_LABELS).to(device)
print(f"Class weights calculados (min={weights.min():.3f}, max={weights.max():.3f})")

# ============================================================
# ENTRENAMIENTO — v2: lr=1e-4, epochs=10, label_smoothing=0.1
# ============================================================
...
# Trainer personalizado con class weights + label smoothing
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        labels = inputs.pop("labels")
        outputs = model(**inputs)
        # Label smoothing manual + class weights
        loss_fct = CrossEntropyLoss(
            weight=weights,
            label_smoothing=0.1   # ← reduce sobreconfianza en clases con pocas muestras (gq, py)
        )
...
training_args = TrainingArguments(
    ...
    # ── MEJORAS v2 ──
    num_train_epochs=10,              # antes 5 → más tiempo para converger
    learning_rate=1e-4,               # antes 2e-4 → más estable
    # ────────────────
...
trainer = WeightedTrainer(
    ...
    callbacks=[EarlyStoppingCallback(early_stopping_patience=4)]  # antes 3
)
...
merged_model = model.merge_and_unload()   # fusiona pesos LoRA en el transformer base; necesario para carga estándar HF por los scripts XAI
...
plt.tight_layout()
plt.savefig(os.path.join(FIG_TRAIN_DIR, "matriz_confusion.png"), dpi=150, bbox_inches='tight')
plt.close()
...
if eval_data:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_TRAIN_DIR, "curva_aprendizaje.png"), dpi=150, bbox_inches='tight')
    plt.close()
...
print("\n" + "="*60)
print("ENTRENAMIENTO COMPLETADO — XLM-T + LoRA v2")
print("="*60)
print(f"""
Modelo:        {MODEL_NAME}
